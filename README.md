...
- **Топ-10 популярных событий** (ключ `events:top10`): кешируется при первом запросе. TTL 5 минут. Инвалидируется при создании, обновлении, удалении события, при изменении мест.
...
- Топ-10 — агрегированный рейтинг, небольшая задержка допустима, поэтому используется TTL, а не активная инвалидация на каждое изменение мест. Это снижает количество операций инвалидации.
