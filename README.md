...
| **Jaeger**     | Сбор и визуализация трейсов          | 16686 (UI), 4317 (OTLP) |
| **Grafana**    | Визуализация метрик (дашборды)       | 3000 |
| **Serilog**    | Структурированное логирование в JSON | -    |

### OpenTelemetry

Во всех трёх сервисах настроена инструментация OpenTelemetry:

1. **Трейсы** — автоматический сбор:
   - Входящие HTTP-запросы
   - Исходящие HTTP-запросы
   - Запросы к базе данных (EF Core)
   
   Экспорт в Jaeger через OTLP.

2. **Метрики** — автоматический сбор:
   - ASP.NET Core метрики (latency, throughput, error rate)
   - Метрики рантайма .NET (GC, память, потоки)
   
   Экспорт в Prometheus через эндпоинт `/metrics`.

3. **Логи** — структурированный JSON-формат через Serilog.
